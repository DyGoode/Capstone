...
class Recipe(db.Model):
    ...
    servings = db.Column(db.Numeric(precision=3), nullable = True)
    user_token = db.Column(db.String, db.ForeignKey('user.token'), nullable = False)

    def __init__(self, title, cuisine, course, servings, user_token, id = ''):
        self.id = self.set_id()
        self.title = title
...
class RecipeSchema(ma.Schema):
    class Meta:
        fields = ['id', 'title', 'cusine', 'course', 'servings']
